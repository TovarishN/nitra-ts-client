...
let length = buf.readInt32LE(0);
				msg.libs = [];
				for (let i = 0; i < length; i++) {
					stack.push(...GetStringDeserializer(msg.libs[i]));
				} }
;
 return retStack;
}
...
let length = buf.readInt32LE(0);
				msg.languageNames = [];
				for (let i = 0; i < length; i++) {
					stack.push(...GetDeserializer(msg.languageNames[i]));
				} }
;
 return retStack;
}
...
let length = buf.readInt32LE(0);
				msg.kinds = [];
				for (let i = 0; i < length; i++) {
					stack.push(...GetDeserializer(msg.kinds[i]));
				} }
;
 return retStack;
}
...
let length = buf.readInt32LE(0);
				msg.fileExtensions = [];
				for (let i = 0; i < length; i++) {
					stack.push(...GetDeserializer(msg.fileExtensions[i]));
				} }
;
 return retStack;
}
...
retStack.push((buf, stack) => { msg.Version = { Value: buf.readInt32LE(0) }; });
retStack.push((buf, stack) => { msg.SolutionId = { Value: buf.readInt32LE(0) }; });
retStack.push((buf, stack) => { msg.type = <PrettyPrintState>buf.readInt32LE(0); });
retStack.push((buf, stack) => {   }); retStack.push((buf, stack) => { msg.text = buf.toString(); }); ;
 return retStack;
}
...
let length = buf.readInt32LE(0);
				msg.ProjectSupprts = [];
				for (let i = 0; i < length; i++) {
					stack.push(...GetDeserializer(msg.ProjectSupprts[i]));
				} }
...
retStack.push((buf, stack) => { msg.Type = <CompilerMessageType>buf.readInt32LE(0); });
...
retStack.push((buf, stack) => { msg.Source = <CompilerMessageSource>buf.readInt32LE(0); });
retStack.push((buf,stack) => {
let length = buf.readInt32LE(0);
				msg.NestedMessages = [];
				for (let i = 0; i < length; i++) {
...
retStack.push((buf, stack) => {   }); retStack.push((buf, stack) => { msg.description = buf.toString(); }); 
retStack.push((buf, stack) => { msg.kind = <ReflectionKind>buf.readInt32LE(0); });
...
retStack.push((buf, stack) => { msg.Kind = <PropertyKind>buf.readInt32LE(0); });
retStack.push((buf, stack) => {   }); retStack.push((buf, stack) => { msg.Name = buf.toString(); }); 
